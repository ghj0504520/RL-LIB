...
class DuelingCritic(nn.Module):
    def __init__(self, hidden_size, num_inputs, num_outputs):
        ...
        self.advantageLayer = nn.Sequential(
            nn.Linear(hidden_size, num_outputs)  # One advantage per action A(s, a)
        )
    ...
    def forward(self, inputs):
        ...
        value = self.valueLayer(features)  # V(s)
        advantage = self.advantageLayer(features)  # A(s, a)
        qvalue = value + (advantage - advantage.mean(dim=1, keepdim=True))  # Q(s, a)
        return qvalue
...
class DQNTARGETAGENT(object):
    def __init__(self, num_inputs, num_actions):
        ...
        self.critic = DuelingCritic(128, num_inputs, num_actions).to(deviceGPU)
        self.critic_target = DuelingCritic(128, num_inputs, num_actions).to(deviceGPU)
...
plt.savefig('Dueling_DQN_plot-whole.png')
plt.show()
