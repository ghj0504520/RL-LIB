...
class PPOCLIPAGENT:
    def __init__(self, num_inputs, action_space, gamma=0.99, hidden_size=64, batch_size=32,
                gae_lambda = 0.95, lr_a=3e-4, lr_c=3e-4, kl_beta = 5, target_kl = 0.03, ppo_epoch = 10, entropy_coef = 0.01):
    ...
    def update1(self, batch,ep):
        ...
        for _ in range(self.ppo_epoch):
            ...
            surr = ratios * advantage
            
            kl_divergence = self.compute_kl(old_log_probs, new_log_probs)
            
            policy_loss = -surr.mean() + self.kl_beta * kl_divergence - self.entropy_coef *  torch.mean(new_act_dist_entropy)  # Trick 5: policy entropy
            # Update actor
...
plt.savefig('PPO_KL_Continuous_plot-whole.png')
plt.show()