...
class DDQNAGENT(object):
    ...
    def update_parameter(self, batch):
        ...
        with torch.no_grad():
            predict_next_q_values = self.critic(next_state_batch)
            # Get the actions with maximum Q-values for the next state
            best_next_actions = predict_next_q_values.max(1)[1]

            # Action evaluation: Use the target network to evaluate the value of the chosen action
            next_q_values = self.critic_target(next_state_batch)
            next_q_value = next_q_values.gather(1, best_next_actions.unsqueeze(1)).squeeze(1)
...
ddqnAgent = DDQNAGENT(env.observation_space.shape[0], env.action_space.n, lr_c)
...
for episode_idx in range(1, total_episodes + 1):
    ...
    while not done :
        step = step+1
        action = ddqnAgent.action_selection(state, epsilon) #select action from updated q net
...
plt.savefig('DDQN_plot-whole.png')
plt.show()
