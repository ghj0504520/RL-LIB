...
class PPOCLIPAGENT():
    def __init__(self, num_inputs, action_space, gamma=0.995, hidden_size=128, batch_size=32,
                gae_lambda = 0.999, lr_a=1e-4, lr_c=1e-3, kl_beta = 0.1, target_kl = 0.01, ppo_epoch = 10):
        self.gamma = gamma
    ...
    def compute_kl(self, old_log_probs, new_log_probs):
        """Compute KL divergence between old and new distributions for categorical actions."""
        old_dist = Categorical(logits=old_log_probs)
        new_dist = Categorical(logits=new_log_probs)
    ...
    def update(self, batch):
        ...
        with torch.no_grad():
            ...
            #old_act_dist = self.actor(states)
        ...
        for _ in range(self.ppo_epoch):
            ...
            kl_divergence = self.compute_kl(old_log_probs, new_log_probs)
            
            policy_loss = -surr.mean() + self.kl_beta * kl_divergence
...
if __name__ == "__main__":
    ...
    agent = PPOCLIPAGENT( env.observation_space.shape[0], env.action_space, gamma=gamma, hidden_size=hidden_size, batch_size=batch_size,
                gae_lambda = gae_lambda , lr_a=lr_a, lr_c=lr_c, kl_beta = kl_beta, target_kl = target_kl, ppo_epoch = ppo_epoch)
    ...
    plt.savefig('PPO_KL_Discrete_plot-whole.png')
    plt.show()