...
class DDPGAGENT(object):
    ...
    def update_parameters(self, batch):
        ...
        with torch.no_grad():
            ...
            next_q_values = self.critic_target(next_state_batch, next_action_batch) # .detach()
        ...
        # update the interaction critic network (with target network)
        # using unsqueeze(1) to avoid shape mismatching
        cur_q_values = self.critic(state_batch, action_batch.unsqueeze(1))
 
        # update the interaction critic network
        value_loss = F.mse_loss(target_q_values, cur_q_values) # .detach()
        ...
        policy_loss = -self.critic(state_batch, self.actor(state_batch)*2)
...
def train():    
    ...
    plt.savefig('DDPG_plot-whole.png')
    plt.show()
